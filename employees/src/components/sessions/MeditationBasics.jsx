...
const useVideoPlayer = (onComplete) => {
  ...
  const handlePlay = () => {
    if (videoRef.current) {
      const stopTracking = startVideoTimeTracking('program', videoRef.current, 'meditationBasics');
...
const MeditationBasics = () => {
  ...
  const { progress, loading: isLoading2, updateProgress, unlockLevel } = useProgress('meditationBasics');
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch('http://localhost:3000/api/programtracker?programType=meditationBasics', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
      ...
      experts: [
        {
          name: "Dr. John Deo",
          photo:
           "https://tse4.mm.bing.net/th?id=OIP.7o6ZXyp5MItVqTrJRJzo8QAAAA&pid=Api&P=0&h=180",
          rating: 4.9,
          reviews: 657,
          description:
