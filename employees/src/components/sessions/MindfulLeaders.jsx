...
const useVideoPlayer = (onComplete) => {
  ...
  const handlePlay = () => {
    if (videoRef.current) {
      const stopTracking = startVideoTimeTracking('program', videoRef.current, 'mindfulLeaders');
...
const MindfulLeaders = () => {
  ...
  const { progress, loading: isLoading2, updateProgress, unlockLevel } = useProgress('mindfulLeaders');
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch('http://localhost:3000/api/programtracker?programType=mindfulLeaders', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
      ...
      experts: [
        {
          name: "Vaibhav Sanka",
          photo:
            "https://visionhospitalgoa.com/wp-content/uploads/2015/05/Dr.Vaibhav-Dukle.jpg",
          rating: 4.9,
          reviews: 657,
          description:
