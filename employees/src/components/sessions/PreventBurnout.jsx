...
const useProgress = (programType) => {
  ...
  const fetchProgress = async () => {
    try {
      ...
      const response = await fetch(`https://ekaant.onrender.com/api/programtracker?programType=${programType}`, {
        headers: {
  ...
  const updateProgress = async (dayIndex, activityIndex, completed) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://ekaant.onrender.com/api/programtracker/update', {
        method: 'POST',
        headers: {
  ...
  const unlockLevel = async (level) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://ekaant.onrender.com/api/programtracker/unlocklevel', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
...
const useVideoPlayer = (onComplete) => {
  ...
  const handlePlay = () => {
    if (videoRef.current) {
      const stopTracking = startVideoTimeTracking('program', videoRef.current, 'preventBurnout');
...
const PreventBurnout = () => {
  ...
  const { progress, loading: isLoading2, updateProgress, unlockLevel } = useProgress('preventBurnout');
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch('https://ekaant.onrender.com/api/programtracker?programType=preventBurnout', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
  ...
  useEffect(() => {
    const updateProgress = async () => {
      try {
        const token = localStorage.getItem('token');
        await fetch('https://ekaant.onrender.com/api/programtracker/update', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
  ...
  const program = {
    ...
    experts: [
      {
        name: "Kupdeep Singh",
        photo:
          "https://tse4.mm.bing.net/th?id=OIP.VFVFnOHFUcnYKkiE58CIIQHaHa&pid=Api&P=0&h=180",
        rating: 4.9,
        reviews: 657,
        description:
    ...
    journey: [
      ...
      },
      {
        level: 2,
        name: "Calm",
        video: "/videos/5 Min Morning Yoga to FEEL AMAZING_(720P_HD).mp4",
        duration: 5,
      },
      {
        level: 3,
        name: "Confidence",
        video: "/videos/10 min Yoga for Beginners - Gentle _ Simple Yoga Stretch(720P_HD).mp4",
        duration: 14,
      },
      {
        level: 4,
        name: "Focus",
        video: "/videos/Guided Morning Meditation _ 10 Minutes To Start Every Day Perfectly ☮(720P_HD).mp4",
        duration: 10,
      },
      {
        level: 5,
        name: "Trust",
        video: "/videos/Impact of 5 minutes meditation on your Daily Life _calm(480P).mp4",
        duration: 5,
      },
      {
        level: 6,
        name: "Optimism",
        video: "/videos/Relaxed Awareness _ 5 Min Guided Meditation(720P_HD)_1.mp4",
        duration: 5,
      },
      {
        level: 7,
        name: "Control",
        video: "/videos/Yoga For Neck_ Shoulders_ Upper Back  _  10-Minute Yoga Quickie(720P_HD).mp4",
        duration: 10,
      },
    ],
  };
          ...
          <Dialog 
            ...
            PaperProps={{
              style: {
                ...
                borderRadius: '16px',
                boxShadow: '0 25px 50px -12px rgba(50, 7, 103, 0)',
                maxWidth: '500px',
                width: '100%',
              }
            }}
