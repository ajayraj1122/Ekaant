...
const useVideoPlayer = (onComplete) => {
  ...
  const handlePlay = () => {
    if (videoRef.current) {
      const stopTracking = startVideoTimeTracking('program', videoRef.current, 'healthyHabits');
  ...
  const { progress, loading: isLoading2, updateProgress, unlockLevel } = useProgress('healthyHabits');
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch('https://ekaant.onrender.com/api/programtracker?programType=healthyHabits', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
      ...
      experts: [
        {
          name: "Dr. Mansi ",
          photo:
            "https://labuwiki.com/wp-content/uploads/2020/12/25.jpg",
          rating: 4.9,
          reviews: 657,
          description:
