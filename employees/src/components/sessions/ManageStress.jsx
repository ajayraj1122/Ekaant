...
const useVideoPlayer = (onComplete) => {
  ...
  const handlePlay = () => {
    if (videoRef.current) {
      const stopTracking = startVideoTimeTracking('program', videoRef.current, 'manageStress');
...
const ManageStress = () => {
  ...
  const { progress, loading: isLoading2, updateProgress, unlockLevel } = useProgress('manageStress');
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch('https://ekaant.onrender.com/api/programtracker?programType=manageStress', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
