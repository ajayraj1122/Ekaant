...
const useVideoPlayer = (onComplete) => {
  ...
  const handlePlay = () => {
    if (videoRef.current) {
      const stopTracking = startVideoTimeTracking('program', videoRef.current, 'stressedToBalanced');
  ...
  const StressedToBalanced = () => {
    ...
    const { progress, loading: isLoading2, updateProgress, unlockLevel } = useProgress('stressedToBalanced');
    ...
    useEffect(() => {
      const fetchProgress = async () => {
        try {
          const token = localStorage.getItem('token');
          const response = await fetch('http://localhost:3000/api/programtracker?programType=stressedToBalanced', {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
      ...
      const updateProgress = async () => {
        try {
          ...
          await fetch('http://localhost:3000/api/programtracker/update', {
            ...
            body: JSON.stringify({
              programType: 'stressedToBalanced',
      ...
      experts: [
        {
          name: "Viraj Singh",
          photo:
            "https://tse4.mm.bing.net/th?id=OIP.Rq6gZO-BJQQsJqRK_QYMUwAAAA&pid=Api&P=0&h=180",
          rating: 4.9,
          reviews: 657,
          description:
 ...
 export default StressedToBalanced;