...
const useVideoPlayer = (onComplete) => {
  ...
  const handlePlay = () => {
    if (videoRef.current) {
      const stopTracking = startVideoTimeTracking('program', videoRef.current, 'cultivateSleep');
...
const CultivateSleep = () => {
  ...
  const { progress, loading: isLoading2, updateProgress, unlockLevel } = useProgress('cultivateSleep');
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch('https://ekaant.onrender.com/api/programtracker?programType=cultivateSleep', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
      ...
      experts: [
        {
          name: "Joane pier",
          photo:
            "https://tse2.mm.bing.net/th?id=OIP.NJB6dPEjNfo276psJ59uAQHaG0&pid=Api&P=0&h=180",
          rating: 4.9,
          reviews: 657,
          description:
