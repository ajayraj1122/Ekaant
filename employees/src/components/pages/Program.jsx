...
const Program = () => {
  ...
  const [programsData, setProgramsData] = useState([]); //State to hold fetched programs
  ...
  useEffect(() => {
    const fetchPrograms = async () => {
      try {
        ...
        setProgramsData(data); // Update state with fetched programs
      } catch (error) {
  ...
  const renderPrograms = (isLive = false) => {
    const filteredPrograms = isLive
      ? programsData.filter((program) => program.expert.sessionTime.includes("Live"))
      : programsData.filter((program) => !program.expert.sessionTime.includes("Live"));
